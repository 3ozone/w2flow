"""Model ORM per a la taula tenders (RF-02, RF-03)."""
...
class TenderModel(Base):
    ...
    cpv_principal: Mapped[str | None] = mapped_column(String(20), nullable=True)
    data_limit: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    durada_dies: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # --- Filtres ---
    tipus_contracte_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
